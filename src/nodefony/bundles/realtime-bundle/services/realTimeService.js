...
const WebSocket = require(path.resolve(__dirname, "..", "src", "webSocket.js"));
const ServiceSocket = require(path.resolve(__dirname, "..", "src", "serviceSocket.js"));
const SpawnSocket = require(path.resolve(__dirname, "..", "src", "spawnSocket.js"));
...
class realTime extends nodefony.Service {
  ...
  initialize () {
    ...
    for (const services in this.settings.services) {
  ...
  handleConnection (message, context) {
    ...
    case "HTTPS":
    case "HTTP2":
      const data = this.protocol.onMessage(message, context);
      const url = nodefony.extend({}, context.request.url);
  ...
  createSocket (type, options, port, domain) {
    ...
    case "WEBSOCKET":
      return this.socketWeb(options);
    case "SERVICESOCKET":
    case "SERVICE":
      return this.serviceSocket(options);
    case "SPAWNSOCKET":
    case "SPAWN":
    case "EXEC":
      return this.spawnSocket(options);
    }
  }
  ...
  async onMessage (message, context) {
    ...
    case "object":
      try {
        ...
        case "/meta/connect":
        {
          const connection = this.connections.setConnection(context);
          this.log(`connect realtime : ${connection.id}`, "INFO");
          message.clientId = connection.id;
          context.once("onClose", (/* code, info*/) => {
            const res = this.connections.removeConnection(connection.id);
            if (res) {
  ...
  onUnSubscribe (message, data /* , context, error*/) {
    try {
      const name = message.subscription.split("/")[2];
      const serv = this.services[name];
      switch (serv.type) {
      case "tcp":
        const client = this.connections.getSocket(message.clientId);
        if (client) {
          client.end(data);
  ...
  onConnect (message, connection) {
    const advice = this.settings.system.reconnect.connect;
    const response = this.protocol.connectResponse(message, advice, {
      address: connection.remote
    });
  ...
  onDisconnect (message, context) {
    const response = this.protocol.disconnectResponse(message);
    this.send(context, this.protocol.send(response));
    context.close();
  }

  onPublish (message) {
    const socket = this.connections.getSocket(message.clientId);
    let error = null;
    if (socket) {
      if (message.data) {
        try {
          return socket.send(message.data)
            .then(() => this.protocol.publishResponse(message.subscription, message.id))
            .catch((e) => {
              this.log(e, "ERROR");
            });
        } catch (e) {
  ...
  onSubscribe (message, data, context) {
    ...
    case "tcp":
    {
      const options = nodefony.extend(true, {}, serv.options || {});
      try {
    ...
    case "servicesocket":
    {
      ...
      try {
        ...
        socket.connect()
          .then((ret) => {
            try {
              this.log(`Try to connect service:   ${name}`, "INFO");
              this.log(options, "DEBUG");
              id = this.connections.setSocket(message.clientId, socket);
              message.successful = true;
              message.clientId = id;
              ...
              this.log(`CONNECT SERVICE : ${message.ext.socket.name} => clientId : ${id}`, "INFO");
              return message;
            } catch (e) {
              throw e;
            }
        ...
        socket.on("message", (data) => {
          this.send(
            context,
            this.protocol.publishMessage(message.subscription, data, message.clientId)
          );
        });
      } catch (e) {
        this.log(e, "ERROR");
        message.error = this.protocol.errorResponse(500, message.subscription, "Create Service realtime error");
        return this.send(context, this.protocol.send(message));
      }
      break;
    }
