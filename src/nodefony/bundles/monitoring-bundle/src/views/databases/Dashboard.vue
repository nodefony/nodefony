<template>
<v-layout v-if="orm" class="w-100 h-100" style="position:absolute">
  <v-navigation-drawer width="250" permanent>
    <v-card class="d-flex" max-width="250" max-height="125" min-height="100" tile>
      <v-img class="mt-3" width="80" height="80" :src="imageOrm" />
    ...
    <v-tabs density="compact" v-model="tab" direction="vertical">
      <v-tab value="dashboard" to="/database">
      ...
      <div v-if="!routeChild">
        <v-tab value="connectors">
          ...
          Connectors
          <v-spacer></v-spacer>
          <v-chip density="compact" inline> {{nbConnectors}}</v-chip>
        </v-tab>
        <v-divider></v-divider>
        <v-tab value="entities">
          ...
          <v-chip density="compact" inline> {{nbEntities}}</v-chip>
        </v-tab>
        <v-divider></v-divider>
      </div>
    </v-tabs>
  ...
  <v-layout v-if="routeChild" class="h-100 w-100 overflow-auto" style="margin-left:250px">
    <router-view name="orm"></router-view>
  </v-layout>
  <v-window v-else v-model="tab" class="h-100 w-100 overflow-auto" style="margin-left:250px">
    <v-window-item value="dashboard">
      <v-container fluid class="w-100 h-100">
        <v-layout fluid class="d-flex flex-row flex-wrap justify-space-around">
          <n-orm-connector widget v-for="(connector) in connectors" :key="connector.name" :connector="connector" min-width="300" max-width="600" min-height="200" class="d-flex flex-column flex-grow-1 flex-shrink-1 my-2 mx-2" />
        </v-layout>

        <v-layout fluid class="d-flex flex-row flex-wrap justify-space-around">
          <n-orm-entity widget skipQuery v-for="(entity) in entities" :key="entity.name" :ormEntity="entity" :name="entity.name" min-width="300" min-height="200" max-width="400" class="d-flex flex-column  flex-grow-1 flex-shrink-1 my-2 mx-2" />
        </v-layout>
      </v-container>
    </v-window-item>
