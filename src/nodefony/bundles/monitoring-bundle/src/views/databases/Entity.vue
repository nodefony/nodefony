  ...
  <v-card v-if="entity"
    ...
    <v-toolbar density="compact"
      ...
      <v-btn color="grey"
             ...
             variant="outlined"
             class="mr-2"
             @click="goToEntity"> {{entity.name}}</v-btn>
    </v-toolbar>
    ...
    <v-list density="compact"
      ...
      <v-list-item class="text-caption"
        ...
        <v-chip class="text-caption"
                density="compact">{{entity.connectorName}}</v-chip>
      </v-list-item>
      <v-divider></v-divider>
      <v-list-item class="text-caption"
                   density="compact">
      ...
      <v-list-item class="text-caption"
        ...
        <v-chip class="text-caption"
                density="compact">{{nbAttributes}}</v-chip>
      </v-list-item>
    </v-list>
    ...
    <v-container v-if="!widget && attributes">
      ...
      <v-table fixed-header
        ...
        <tbody>
          <tr v-for="(item, key) in properties"
              :key="key">
            <td>{{ key }}</td>
            <td>{{ item.type }}</td>
  ...
  apollo: {
    request: {
      ...
      query: gql`
	    ...
	    `,
      update: (data) => JSON.parse(data.entity),
      // Reactive parameters
      variables () {
