<template >
<v-container v-if="routes" fluid class="w-100 pa-0 h-100" style="position:absolute">

  <v-toolbar color="#233056" theme="dark" extended flat>
    <v-app-bar-title>
      Router
      <v-badge color="info" v-if="nbRoutes" :content="nbRoutes" inline>
      </v-badge>
    </v-app-bar-title>
    ...
    <template v-slot:append>
      <v-icon size="50" color="#43853d" rounded="0">mdi-protocol</v-icon>
    </template>
  </v-toolbar>

  <v-table fixed-header class="overflow-auto" style="height:calc(100% - 30px)">
    <thead>
      <tr>
        <th class="text-left">
          index
