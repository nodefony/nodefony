...
const browserStorage = class browserStorage {
  ...
  get (key) {
    const ele = this.data.getItem(key);
...
const load = function () {
  ...
  const state = this.storage.get("nodefony_debug");
  if (state === false) {
...
class Nodefony {
  ...
  monitoringWorkbox (registration) {
    ...
    switch (true) {
    case Boolean(registration.installing):
      serviceWorker = registration.installing;
      break;
    case Boolean(registration.waiting):
      serviceWorker = registration.waiting;
      break;
    case Boolean(registration.active):
