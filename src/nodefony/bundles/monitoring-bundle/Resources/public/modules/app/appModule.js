...
stage.registerModule("app", function() {
	...
	var module = class module  extends stage.Module {
		...
		initialize () {
			...
			try {
				var server = "/nodefony/socket";
				this.realtime = new nodefony.Socket();
				this.realtime.on("handshake", (message, socket) => {
					//this.logger("handshake", "INFO");
				});
				this.realtime.on("ready", (message, socket) => {
					//this.logger(`ready`);
				});
				this.realtime.on("disconnect", (message, socket) => {
					//this.logger("disconnect", "INFO");
				});
				this.realtime.on("subscribe", (service, message, socket) => {
					//this.logger(`Socket subscribe ${service}`)
				});
				this.realtime.on("unsubscribe", (service, message, socket) => {
					//this.logger(`Socket unsubscribe : ${service}`)
				});
				this.realtime.on("connect", (message, socket) => {
					//this.logger(`Socket connect`)
				});
				this.realtime.on("error", (code, args, message) => {
