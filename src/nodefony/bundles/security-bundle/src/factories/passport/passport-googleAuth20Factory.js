  ...
  class googleFactory extends nodefony.passeportFactory {
    ...
    getStrategy (options) {
      return new Promise((resolve, reject) => {
        try {
          const strategy = new GoogleStrategy(options, (accessToken, refreshToken, profile, cb) => {
            this.log(`TRY AUTHENTICATION ${this.name}`, "DEBUG");
            const mytoken = new nodefony.security.tokens.google(profile, accessToken, refreshToken);
