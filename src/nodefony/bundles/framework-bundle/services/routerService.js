  ...
  const annotations = function (file, bundle, parser, callback) {
    try {
      return this.readFile(file)
        .then((fileContent) => this.annotations.parseController(fileContent, bundle, file, parser)
          .then((obj) => {
...
module.exports = class router extends nodefony.Service {
  constructor (container) {
    ...
    this.reader = (function (context) {
      const func = context.container.get("reader").loadPlugin("routing", pluginReader);
  ...
  generatePath (name, variables, host) {
    try {
      ...
      const route = this.getRoute(name.replace(/\s/g, ""));
      const queryString = variables ? variables.queryString : null;
      if (!route) {
  ...
  resolve (context) {
    const resolver = new nodefony.Resolver(context, this);
  ...
  nodeReader (filePath, bundle, obj) {
    for (const route in obj) {
      try {
        const newRoute = new nodefony.Route(route);
        newRoute.filePath = filePath;
        newRoute.bundle = bundle;
        for (const ele in obj[route]) {
          const arg = obj[route][ele];
          switch (ele) {
          case "pattern":
