...
nodefony.register.call(nodefony.context, "websocket", function(){
  ...
  const onClose = function(reasonCode, description){
    ...
    if ( ! this.request ){
      ...
      this.logger('CLOSE : ' + this.connection.remoteAddress  + " ORIGIN : "+this.request.origin  +" " +reasonCode +" " + description , "INFO");
    }
    if (this.connection.state !== "closed"){
      try {
        ...
        this.logger( 'CLOSE : ' + this.connection.remoteAddress  + " ORIGIN : "+this.request.origin  +" " +e.message , "ERROR");
      }
    }else{
      this.fire("onClose", reasonCode, description, this.connection);
    }
  ...
  const websocket = class websocket extends nodefony.Service {
    ...
    constructor (container, request, type){
      ...
      this.logger("FROM : "+ this.remoteAddress +" HOST : "+this.domain+" URL : "+this.url, "INFO");
      // session
      this.session = null;
