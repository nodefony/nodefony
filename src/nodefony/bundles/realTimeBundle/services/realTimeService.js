...
module.exports = nodefony.registerService("realTime", function(){
  ...
  */
  ...
  const realTime = class realTime extends nodefony.syslog {
    ...
    onHandshake (message, connectionId, ipClient){
      ...
      let response = this.protocol.handshakeResponse(message, advice,{address:ipClient}) ;
...
onConnect (message, ipClient){
  ...
  let response = this.protocol.connectResponse(message, advice, {address:ipClient});
