...
module.exports = class sessionController extends nodefony.controller {
  constructor (container, context) {
    ...
    const {type} = this.context.resolver.variables;
  ...
  async sessionAction (type, message) {
    ...
    switch (type) {
    case "start":
      return this.sessionService.start(this.context).then((session) => this.renderJson({
        id: session.id,
   ...
   */
  attributesAction () {
    const kernelMeta = this.session.getMetaBag("kernel");
    if (!kernelMeta) {
