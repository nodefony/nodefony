...
module.exports = class redisController extends nodefony.controller {
  ...
  async indexAction() {
    ...
    let result = await this.main.hGetAll('key');
    await this.main.hSet('key', 'sboob', 'cci');
    let result2 = await this.main.hGetAll('key');
    console.log(result, result2)
    let message = {
      foo: "bar"
    }
    try {
      let res = await this.publisher.publish("send", JSON.stringify(message));
      console.log(res)
